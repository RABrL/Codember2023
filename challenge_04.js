...
(async() => {
  ...
  const realFiles = files.filter(({checksum,id}) => {
    const map = new Map()
    ...
    for(const [key,value] of map) {
      if(value === 1) uniqueLet += key
    }
    
    return checksum === uniqueLet
  })
  
  console.log(realFiles[32].checksum)
